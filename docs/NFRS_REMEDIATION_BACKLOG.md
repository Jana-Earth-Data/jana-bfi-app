...
| ☑ | N0.10 | **Fixed (PR0-a).** Added `scripts/check-regulatory-boundary.mjs` (8th `prebuild` guard, run by CI). Fails on any *new* policy constant/duplicated aggregator outside `lib/regulatory`; grandfathers the known occurrences (warn-only) so PR0-b/c tighten the net as each relocates. N0.6's `npr-per-usd` entry was removed when it landed (PR0-b); three remain (N0.5/N0.1/N0.7). See PROJECT_PLAN §13. | Prevents regression of N0.1–N0.7 | `scripts/check-*.mjs` | — | — | 1 |
