...
| ☑ | N0.10 | **Fixed (PR0-a).** Added `scripts/check-regulatory-boundary.mjs` (8th `prebuild` guard, run by CI). Fails on any *new* policy constant/duplicated aggregator outside `lib/regulatory`; grandfathers the known occurrences (warn-only) so PR0-b/c tighten the net as each relocates. N0.6's `npr-per-usd`, N0.7's `as-of-date`, and N0.2's `ev-floor-million` entries were removed as those landed (PR0-b), and N0.1 added a `duplicate-aggregator` (`function buildSummary`) pattern that fails immediately; one grandfather remains (N0.5 retail factor). See PROJECT_PLAN §13. | Prevents regression of N0.1–N0.7 | `scripts/check-*.mjs` | — | — | 1 |
