...
| ☑ | N0.10 | **Fixed (PR0-a).** Added `scripts/check-regulatory-boundary.mjs` (8th `prebuild` guard, run by CI). Fails on any *new* policy constant/duplicated aggregator outside `lib/regulatory`; grandfathers the four known N0.1/N0.5/N0.6/N0.7 occurrences (warn-only) so PR0-b/c tighten the net as each relocates. See PROJECT_PLAN §13. | Prevents regression of N0.1–N0.7 | `scripts/check-*.mjs` | — | — | 1 |
